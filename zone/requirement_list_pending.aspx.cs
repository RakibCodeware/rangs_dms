...
public partial class requirement_list_pending : System.Web.UI.Page
{
    ...
    private void fnLoadData()
    {
        ...
        sSql = sSql + " SUM(ISNULL(dbo.RequirmentDetails.ReqQty, 0)) AS tQty, dbo.RequirmentMaster.Delivered, ";
        sSql = sSql + " dbo.RequirmentMaster.Remarks, dbo.RequirmentMaster.ReqTo, dbo.RequirmentMaster.ReqFor, ";
        sSql = sSql + " dbo.RequirmentMaster.DealerCode, dbo.RequirmentMaster.DealerID, dbo.RequirmentMaster.EID,";
        sSql = sSql + " dbo.Entity.eName, dbo.Entity.EntityCode";
        sSql = sSql + " FROM  dbo.RequirmentDetails INNER JOIN";
        sSql = sSql + " dbo.RequirmentMaster ON dbo.RequirmentDetails.ReqAID = dbo.RequirmentMaster.ReqAID LEFT OUTER JOIN";
        sSql = sSql + " dbo.Entity ON dbo.RequirmentMaster.EID = dbo.Entity.EID";

        //sSql = sSql + " WHERE (dbo.RequirmentMaster.Delivered = 0)";
        sSql = sSql + " WHERE (dbo.Entity.EntityCode = '" + Session["sBrCode"].ToString() + "')";

        if (txtInvNo.Text.Length > 0)
        {
            sSql = sSql + " AND (dbo.RequirmentMaster.ReqNo LIKE '%" + txtInvNo.Text + "%')";
        ...
        else
        {
            ...
            {
                sSql = sSql + " AND (dbo.RequirmentMaster.ReqFor ='" + this.ddlEntity.SelectedItem.Text + "')";
            }

        }

        sSql = sSql + " GROUP BY dbo.RequirmentMaster.ReqAID, dbo.RequirmentMaster.ReqNo, dbo.RequirmentMaster.ReqDate,";
        sSql = sSql + " dbo.RequirmentMaster.Delivered, dbo.RequirmentMaster.Remarks, dbo.RequirmentMaster.ReqTo, ";
        sSql = sSql + " dbo.RequirmentMaster.ReqFor, dbo.RequirmentMaster.DealerCode, dbo.RequirmentMaster.DealerID,";
        sSql = sSql + " dbo.RequirmentMaster.EID, dbo.Entity.eName, dbo.Entity.EntityCode";

        sSql = sSql + " ORDER BY dbo.RequirmentMaster.ReqDate, dbo.RequirmentMaster.ReqAID";
    ...
    //LOAD SALES SUMMARY CHALLAN WISE PENDING LIST
    private void fnLoadData_Pending()
    {
        ...
        //sSql = sSql + " WHERE (dbo.RequirmentMaster.Delivered = 0)";
        //sSql = sSql + " AND (dbo.Entity.EntityCode = '" + Session["sBrCode"].ToString() + "')";
        ...
        sSql ="SELECT dbo.Entity.EID, dbo.Entity.eName AS SubStoreName, dbo.RequirmentMaster.ReqAID, ";
        sSql = sSql + " dbo.RequirmentMaster.ReqNo, dbo.RequirmentMaster.ReqDate, dbo.RequirmentMaster.DealerID, ";
        sSql = sSql + " dbo.RequirmentMaster.DealerCode, dbo.RequirmentMaster.ReqFor, dbo.RequirmentMaster.Remarks,";
        sSql = sSql + " Entity_1.eName AS ZoneName, SUM(dbo.RequirmentDetails.ReqQty) AS tQty, dbo.RequirmentMaster.Delivered,";
        sSql = sSql + " Entity_2.eName AS CTPName";
        sSql = sSql + " FROM dbo.Entity INNER JOIN";
        sSql = sSql + " dbo.Entity AS Entity_1 ON dbo.Entity.ZoneID = Entity_1.EID INNER JOIN";
        sSql = sSql + " dbo.Entity AS Entity_2 ON Entity_1.EID = Entity_2.ParentEntity INNER JOIN";
        sSql = sSql + " dbo.RequirmentMaster ON Entity_2.EID = dbo.RequirmentMaster.EID INNER JOIN";
        sSql = sSql + " dbo.RequirmentDetails ON dbo.RequirmentMaster.ReqAID = dbo.RequirmentDetails.ReqAID";
        
        sSql = sSql + " WHERE (dbo.Entity.EntityType = 'Sub Store') AND (dbo.RequirmentMaster.Delivered = 0)";
        sSql = sSql + " AND  (dbo.Entity.EID = '" + Session["sBrId"] + "')";
        if (this.ddlEntity.SelectedItem.Text != "ALL")
        {
            sSql = sSql + " AND (dbo.RequirmentMaster.ReqFor ='" + this.ddlEntity.SelectedItem.Text + "')";
        }

        sSql = sSql + " GROUP BY dbo.Entity.EID, dbo.Entity.eName, dbo.RequirmentMaster.ReqAID,";
        sSql = sSql + " dbo.RequirmentMaster.ReqNo, dbo.RequirmentMaster.ReqDate, dbo.RequirmentMaster.DealerID, ";
        sSql = sSql + " dbo.RequirmentMaster.DealerCode, dbo.RequirmentMaster.ReqFor, dbo.RequirmentMaster.Remarks,";
        sSql = sSql + " dbo.RequirmentMaster.Delivered,Entity_1.eName, Entity_2.eName";
    ...
    */
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        ...
        sSql = sSql + " WHERE (dbo.RequirmentMaster.Delivered = 0)";
        sSql = sSql + " AND (dbo.RequirmentMaster.ReqAID = '" + sPID + "')";
        ...
        if (dr.Read())
        {
            this.lblID.Text = dr["ReqAID"].ToString();
            this.lblInv.Text = dr["ReqNo"].ToString();
            this.lblDate.Text = dr["ReqDate"].ToString();

            this.lblCustName.Text = dr["ReqFor"].ToString();
            this.lblContact.Text = dr["DealerCode"].ToString();
            this.lblDealerID.Text = dr["DealerID"].ToString();
        ...
        sSql = "";
        sSql = "SELECT  dbo.RequirmentMaster.ReqAID, dbo.RequirmentMaster.ReqNo, dbo.RequirmentMaster.ReqDate,";
        sSql = sSql + " dbo.Product.ProductID, dbo.Product.Code, dbo.Product.ProdName, dbo.Product.Model, ";
        sSql = sSql + " dbo.RequirmentDetails.ReqQty, dbo.RequirmentDetails.ProdRemarks, dbo.Product.UnitPrice,";
        sSql = sSql + " dbo.Product.Measure, dbo.Product.DealerPrice, dbo.RequirmentMaster.Delivered, ";
        sSql = sSql + " dbo.RequirmentMaster.Remarks, dbo.RequirmentMaster.ReqTo, dbo.RequirmentMaster.ReqFor,";
        sSql = sSql + " dbo.RequirmentMaster.DealerCode, dbo.RequirmentMaster.DealerID,";
        sSql = sSql + " dbo.RequirmentMaster.EID, dbo.Entity.eName, dbo.Entity.EntityCode";
        ...
        sSql = sSql + " LEFT OUTER JOIN dbo.Entity ON dbo.RequirmentMaster.EID = dbo.Entity.EID";
        sSql = sSql + " WHERE (dbo.RequirmentMaster.ReqAID = '" + this.lblID.Text + "' )";
    ...
    protected void btnDeclar_Click(object sender, EventArgs e)
    {
        ...
        //*************************************************************************************************************
        //AUTO DELIVERY -------------------------------------------------------
        SqlConnection connDMS1 = DBConnection.GetConnection();

        sSql = "";
        sSql = "SELECT * FROM MRSRMaster Where MRSRCode='" + lblInv.Text + "'";
        SqlCommand cmd = new SqlCommand(sSql, conn);
        conn.Open();
        SqlDataReader dr = cmd.ExecuteReader();
        if (dr.Read())
        {
            ...
            sSql = "INSERT INTO MRSRMaster(MRSRCode,TDate,TrType," +
                   ...
                   " )" +
                " Values ('" + dr["MRSRCode"].ToString() + 'D' + "','" + dr["TDate"].ToString() + "','2'," +
                " '" + dr["MRSRCode"].ToString() + "','" + dr["OutSource"].ToString() + "','" + Session["sStoreID"] + "'," +
                " '" + dr["PayAmnt"].ToString() + "','" + dr["DueAmnt"].ToString() + "','" + dr["PayMode"].ToString() + "'," +
                " '" + dr["Customer"].ToString() + "', '" + Session["UserName"] + "', '" + DateTime.Now + "'," +
                " '" + dr["NetSalesAmnt"].ToString() + "','" + dr["TermsCondition"].ToString() + "'," +
                " '" + dr["CashAmnt"].ToString() + "','" + dr["CardAmnt1"].ToString() + "','" + dr["CardAmnt2"].ToString() + "'," +
                " '" + dr["CardNo1"].ToString() + "','" + dr["CardNo2"].ToString() + "','" + dr["CardType1"].ToString() + "','" + dr["CardType2"].ToString() + "'," +
                " '" + dr["Bank1"].ToString() + "','" + dr["Bank2"].ToString() + "','" + dr["SecurityCode"].ToString() + "','" + dr["SecurityCode2"].ToString() + "'," +
                " '" + dr["AppovalCode1"].ToString() + "','" + dr["AppovalCode2"].ToString() + "','" + dr["OnLineSales"].ToString() + "'," +
                " '" + dr["Authorby"].ToString() + "','" + dr["Issby"].ToString() + "'," +
                " '" + dr["DeliveryFrom"].ToString() + "','" + dr["Remarks"].ToString() + "'," +
                " '" + dr["Tag"].ToString() + "','" + dr["RefCHNo"].ToString() + "','" + dr["POCode"].ToString() + "'" +
            " )";
            //" CAST(" + this.lblNetAmnt.Text + " AS Numeric)";  
            // " )";
            SqlCommand cmdDMS = new SqlCommand(sSql, connDMS1);
            connDMS1.Open();
        ...
        long iMRSRID = 0;
        long iMRSRIDdms = 0;
        //RETRIVE MASTER ID FROM DMS FOR AUTO DELIVERY        
        sSql = "";
        sSql = "SELECT MRSRMID FROM MRSRMaster" +
            " WHERE MRSRCode='" + this.lblInv.Text + 'D' + "'" +
            //" WHERE MRSRCode='" + this.txtMRSR.Text + 'D' + "'" +
            " AND TrType=2";
        SqlCommand cmdDMS1 = new SqlCommand(sSql, connDMS1);
        connDMS1.Open();
        dr = cmdDMS1.ExecuteReader();
        if (dr.Read())
        {
        ...
        connDMS1.Close();
        //----------------------------------------------------------------------------
        //RETRIVE MASTER ID FROM DMS SALES       
        sSql = "";
        sSql = "SELECT MRSRMID FROM MRSRMaster" +
            " WHERE MRSRCode='" + this.lblInv.Text + "'" +
            //" WHERE MRSRCode='" + this.txtMRSR.Text + 'D' + "'" +
            " AND TrType=3";
        cmdDMS1 = new SqlCommand(sSql, connDMS1);
        connDMS1.Open();
        dr = cmdDMS1.ExecuteReader();
        if (dr.Read())
        {
        ...
        //----------------------------------------------------------------------------
        sSql = "";
        sSql = "SELECT * FROM MRSRDetails Where MRSRMID='" + iMRSRID + "'";
        ...
        while (dr1.Read())
        {
            ...
            gSql = "INSERT INTO MRSRDetails(MRSRMID,ProductID,Qty," +
                 ...
                 " VALUES('" + iMRSRIDdms + "','" + dr["ProductID"].ToString() + "','" + dr["Qty"].ToString() + "'," +
                 " '" + dr["MRP"].ToString() + "','" + dr["UnitPrice"].ToString() + "','" + dr["TotalAmnt"].ToString() + "','" + dr["DiscountAmnt"].ToString() + "'," +
                 " '" + dr["SLNO"].ToString() + "','" + dr["ProdRemarks"].ToString() + "','" + dr["DisCode"].ToString() + "','" + dr["DisRef"].ToString() + "'," +
                 " '" + dr["WithAdjAmnt"].ToString() + "','" + dr["RetPrice"].ToString() + "','" + dr["NetAmnt"].ToString() + "'," +
                 " '" + dr["BLIPAmnt"].ToString() + "','" + dr["IncentiveAmnt"].ToString() + "','" + dr["IncentiveType"].ToString() + "'," +
                 " '" + dr["CustShowPrice"].ToString() + "','" + Session["sDealerMktEID"] + "','" + Session["sStoreID"] + "')";
            cmdIns = new SqlCommand(gSql, connDMS1);
