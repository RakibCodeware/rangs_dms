...
public partial class Search_Statement_Deposit : System.Web.UI.Page
{
    ...
    private double runningTotalRocket = 0;
    private double runningTotalNagad = 0;
    private double runningTotalJV = 0;
    ...
    private void fnLoadData()
    {
        ...
        sSql = "SELECT CONVERT(varchar(12), DDate, 105) AS DDate," +
            ...
            " ISNULL(RoketAmnt,0) AS RoketAmnt, ISNULL(NagadAmnt,0) AS NagadAmnt, ISNULL(JVAmnt,0) AS JVAmnt," +
            " ISNULL(CashAmnt,0) + ISNULL(CardAmnt,0) +  ISNULL(OnlinePaymentAmnt,0) + " +
            " ISNULL(RoketAmnt,0) + ISNULL(NagadAmnt,0) +  ISNULL(JVAmnt,0) + " +
    ...
    private void fnLoadStatementData()
    {
        ...
        for (DateTime dateTime = startDate; dateTime <= stopDate; dateTime = dateTime.AddDays(1))
        {
            ...
            // ROCKET DEPOSIT AMOUNT
            sSql = "";
            sSql = "SELECT EID, SUM(ISNULL(DepositAmnt,0)) AS DAmnt";
            sSql = sSql + " FROM dbo.tbDeposit";
            sSql = sSql + " WHERE (DepositDate = '" + dateTime + "')";
            sSql = sSql + " AND (EID = '" + Session["EID"] + "')";
            sSql = sSql + " AND (DepositType = 'Rocket')";
            sSql = sSql + " GROUP BY EID";
            cmd = new SqlCommand(sSql, con);
            con.Open();
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
                sSql = "";
                sSql = "UPDATE Temp_DepositStatement ";
                sSql = sSql + " SET RocketAmnt='" + dr["DAmnt"] + "'";
                sSql = sSql + " WHERE DDate='" + dateTime + "'";
                sSql = sSql + " AND UserID='" + Session["UserName"] + "' AND EID='" + Session["EID"] + "'";
            ...
            sSql = sSql + " WHERE (DepositDate = '" + dateTime + "')";
            sSql = sSql + " AND (EID = '" + Session["EID"] + "')";
            sSql = sSql + " AND (DepositType = 'Nagad')";
            sSql = sSql + " GROUP BY EID";
            cmd = new SqlCommand(sSql, con);
            con.Open();
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
                sSql = "";
                sSql = "UPDATE Temp_DepositStatement ";
                sSql = sSql + " SET NagadAmnt='" + dr["DAmnt"] + "'";
                sSql = sSql + " WHERE DDate='" + dateTime + "'";
                sSql = sSql + " AND UserID='" + Session["UserName"] + "' AND EID='" + Session["EID"] + "'";
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            CalcTotal_Rocket(e.Row.Cells[8].Text);
            CalcTotal_Nagad(e.Row.Cells[9].Text);
            CalcTotal_JV(e.Row.Cells[10].Text);

            CalcTotal_TP(e.Row.Cells[11].Text);
        ...
        else if (e.Row.RowType == DataControlRowType.Footer)
        {
            ...
            e.Row.Cells[11].Text = runningTotalTP.ToString("0,0", CultureInfo.InvariantCulture);
    ...
    //CALCULATE TOTAL ROCKET
    private void CalcTotal_Rocket(string _price)
    {
        try
        {
            runningTotalRocket += Double.Parse(_price);
        }
        catch (Exception ex)
        {
            HttpContext.Current.Response.Write(ex.ToString());
    ...
    //CALCULATE TOTAL NAGAD
    private void CalcTotal_Nagad(string _price)
    {
        try
        {
            runningTotalNagad += Double.Parse(_price);
        }
        catch (Exception ex)
        {
            HttpContext.Current.Response.Write(ex.ToString());
    ...
    //CALCULATE TOTAL JV
    private void CalcTotal_JV(string _price)
    {
        try
        {
            runningTotalJV += Double.Parse(_price);
        }
        catch (Exception ex)
        {
            HttpContext.Current.Response.Write(ex.ToString());
