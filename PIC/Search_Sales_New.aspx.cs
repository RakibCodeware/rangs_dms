...
public partial class Search_Sales_New : System.Web.UI.Page
{
    ...
    //    if (dr.Read())
    //    {
    //        iMRSRIDD = Convert.ToInt32(dr["MRSRMID"].ToString());
    ...
    //    sSql = "";
    //    sSql = "DELETE FROM MRSRMaster";
    //    sSql = sSql + " WHERE MRSRMID='" + iMRSRIDD + "'";
    ...
    //    cmd1 = new SqlCommand(sSql, connDSM);
    //    connDSM.Open();
    //    cmd1.ExecuteNonQuery();
    //    connDSM.Close();
    //    //------------------------------------------------------------
    ...
    {
        //LOAD DATA IN GRID
        if (Session["UserName"].ToString() == "Emran" || Session["UserName"].ToString() == "emran")
        {
            fnLoadData();
        }
