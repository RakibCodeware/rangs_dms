...
public partial class Search_Stock : System.Web.UI.Page
{
    ...
    //LOAD DATA IN GRID 
    private void fnLoadData_CTPWise()
    {
        ...
        string sSql = "";
        sSql = "SELECT ISNULL(dbo.VW_Model_IN.QtyIN, 0) AS QtyIN, ISNULL(dbo.VW_Model_OUT.QtyOut, 0) AS OutQty,";
        sSql = sSql + " ISNULL(dbo.VW_Model_IN.QtyIN, 0) - ISNULL(dbo.VW_Model_OUT.QtyOut, 0) AS bQty, ";
        sSql = sSql + " dbo.VW_Product_MRP_CampaignPrice.Code, dbo.VW_Product_MRP_CampaignPrice.Model,";
        sSql = sSql + " dbo.VW_Product_MRP_CampaignPrice.ProdName, dbo.VW_Product_MRP_CampaignPrice.GroupName, ";
        sSql = sSql + " dbo.VW_Product_MRP_CampaignPrice.UnitPrice, dbo.VW_Product_MRP_CampaignPrice.CampaignPrice,";
        sSql = sSql + " dbo.VW_Product_MRP_CampaignPrice.FOBPriceUSD, dbo.VW_Product_MRP_CampaignPrice.ProductID";
        sSql = sSql + " FROM dbo.VW_Product_MRP_CampaignPrice LEFT OUTER JOIN";
        sSql = sSql + " dbo.VW_Model_OUT ON dbo.VW_Product_MRP_CampaignPrice.Model = dbo.VW_Model_OUT.Model LEFT OUTER JOIN";
        sSql = sSql + " dbo.VW_Model_IN ON dbo.VW_Product_MRP_CampaignPrice.Model = dbo.VW_Model_IN.Model";

        sSql = sSql + " WHERE ISNULL(dbo.VW_Model_IN.QtyIN, 0) - ISNULL(dbo.VW_Model_OUT.QtyOut, 0) <> 0";

        if (ddlCategory.SelectedItem.Text != "ALL")
        {
            sSql = sSql + " AND VW_Product_MRP_CampaignPrice.GroupName='" + ddlCategory.SelectedItem.Text + "'";
        }

        if (txtModel.Text != "")
        {
            sSql = sSql + " AND VW_Product_MRP_CampaignPrice.Model='" + txtModel.Text + "'";
        }

        sSql = sSql + " dbo.VW_Product_MRP_CampaignPrice.GroupName, dbo.VW_Product_MRP_CampaignPrice.Model";
    ...
    private void fnLoadStatementData()
    {
        ...
        {
            gSQL = gSQL + " AND  (dbo.Entity.eName='" + this.ddlEntity.SelectedItem.Text + "') ";
        }
