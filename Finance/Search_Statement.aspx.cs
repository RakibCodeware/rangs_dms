...
public partial class Search_Statement : System.Web.UI.Page
{
    ...
    private void fnLoadData()
    {
        ...
        sSql = sSql + " (ISNULL(OBSales,0) - ISNULL(OBWith,0) -ISNULL(OBDeposit,0)) AS OBAmnt,";
        sSql = sSql + " ISNULL(SalesAmnt,0) AS SalesAmnt, ISNULL(WithAmnt,0) AS WithAmnt, ISNULL(DepositAmnt,0) AS DepositAmnt,";
        sSql = sSql + " ((ISNULL(OBSales,0) - ISNULL(OBWith,0) -ISNULL(OBDeposit,0)) + ";
        sSql = sSql + " ISNULL(SalesAmnt,0) - ISNULL(WithAmnt,0) - ISNULL(DepositAmnt,0)) AS CBAmnt";

        sSql = sSql + " FROM Temp_DepositStatement ";

        sSql = sSql + " WHERE (UserID = '" + Session["UserName"] + "') ";
    ...
    private void fnLoadStatementData()
    {
        ...
        */
        ...
        for (DateTime dateTime = startDate; dateTime <= stopDate; dateTime = dateTime.AddDays(1))
        {
            ...
            sSql = "";
            sSql = "INSERT INTO Temp_DepositStatement(";
            sSql = sSql + " DDate,UserID";
            sSql = sSql + " )";
            sSql = sSql + " VALUES(";
            sSql = sSql + " '" + dateTime + "','" + Session["UserName"] + "'";
            sSql = sSql + " )";
            ...
            sSql = "";
            sSql = "SELECT TrType, SUM(ISNULL(NetSalesAmnt,0)) AS tAmnt";
            sSql = sSql + " FROM dbo.MRSRMaster";
            sSql = sSql + " WHERE (TDate >= '7/1/2020') AND (TDate < '" + dateTime + "')";
            sSql = sSql + " AND (TrType = 3)";
            sSql = sSql + " GROUP BY TrType";
            SqlCommand cmd = new SqlCommand(sSql, con);
            con.Open();
            SqlDataReader dr = cmd.ExecuteReader();
            if (dr.Read())
            {
                ...
                sSql = sSql + " AND UserID='" + Session["UserName"] + "'";
            ...
            sSql = "";
            sSql = "SELECT TrType, SUM(ISNULL(NetSalesAmnt,0)) AS tAmnt, TDate";
            sSql = sSql + " FROM dbo.MRSRMaster";
            ...
            sSql = "";
            sSql = "SELECT SUM(ISNULL(DepositAmnt,0)) AS DAmnt";
            sSql = sSql + " FROM dbo.tbDeposit";
            sSql = sSql + " WHERE (DepositDate >= '7/1/2020') AND (DepositDate < '" + dateTime + "')";
            //sSql = sSql + " AND (EID = '" + Session["sBrId"] + "')";
