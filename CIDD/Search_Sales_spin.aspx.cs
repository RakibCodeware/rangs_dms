...
public partial class Search_Sales_spin : System.Web.UI.Page
{
    ...
    private void fnLoadData()
    {
        ...
        sSql = sSql + " dbo.tbCustomer.EID, dbo.Entity.eName, ";
        sSql = sSql + " dbo.tbProduct.MRP, dbo.tbCustomer.DisPer, (dbo.tbProduct.MRP - (dbo.tbCustomer.DisPer/100 * dbo.tbProduct.MRP)) as sp , ";
        sSql = sSql + "  CASE dbo.tbCustomer.sTag WHEN 0 THEN 'Pending Toss' ELSE 'Toss & Win Done' END AS sStatus,";
        sSql = sSql + " dbo.tbCustomer.sTag, dbo.tbCustomer.EntryTime,";
        sSql = sSql + " CONVERT(varchar(12), dbo.tbCustomer.EntryDate, 105) AS EntryDate";

        sSql = sSql + " FROM  dbo.tbCustomer INNER JOIN";
        sSql = sSql + " dbo.tbProduct ON dbo.tbCustomer.ProductID = dbo.tbProduct.ProductID";
        sSql = sSql + " INNER JOIN  dbo.Entity ON dbo.tbCustomer.EID = dbo.Entity.EID";

        //sSql = sSql + " WHERE dbo.tbCustomer.EID='" + Session["EID"].ToString() + "'";
        sSql = sSql + " WHERE (dbo.tbCustomer.EntryDate >= '" + Convert.ToDateTime(this.txtFrom.Text) + "'";
    ...
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        Session["sChNoSpin"] = GridView1.DataKeys[gRow.RowIndex].Value.ToString();
        //Session["sBillNo"] = this.txtInvoiceNo.Text;
        //Session["sReportType"] = "RPT_Sales_Bill";

        Response.Redirect("Sales_Bill_Print_spin.aspx");
