...
public partial class Search_Stock : System.Web.UI.Page
{
    ...
    private void fnLoadData()
    {
        ...
        //    " SUM(ISNULL(QtyIN,0)) - SUM(ISNULL(QtyOut,0)) AS bQty" +
        ...
        //    " GROUP BY GroupName, Model, ProdName" +
        //    " ORDER BY GroupName, Model";
        
        sSql ="SELECT GroupName, Model, ProdName, SUM(QtyIN) AS tIn, SUM(QtyOut) AS tOut,";
        sSql = sSql + " SUM(QtyIN) - SUM(QtyOut) AS bQty, UserID";
        sSql = sSql + " FROM dbo.RPTBalance_Online";
        sSql = sSql + " GROUP BY GroupName, Model, ProdName, UserID";
        sSql = sSql + " HAVING (UserID = '" + Session["UserName"] + "') AND (SUM(QtyIN) - SUM(QtyOut) <> 0)";
        sSql = sSql + " ORDER BY GroupName, Model";
