...
public partial class search_dealer_statement : System.Web.UI.Page
{
    ...
    protected void fnLoadDealerStatement()
    {
        ...
        if (ddlArea.SelectedItem.Text != "ALL")
        {
            gSQL = gSQL + " AND dbo.[Zone].CatName='" + ddlArea.SelectedItem.Text + "'";
        }
        ...
        if (ddlArea.SelectedItem.Text != "ALL")
        {
            gSQL = gSQL + " AND dbo.VW_Delear_Info.ZoneName='" + ddlArea.SelectedItem.Text + "'";
        }
        if (ddlEntity.SelectedItem.Text != "ALL")
        {
        ...
        if (ddlArea.SelectedItem.Text != "ALL")
        {
            gSQL = gSQL + " AND ZoneName='" + ddlArea.SelectedItem.Text + "'";
        }
        if (ddlEntity.SelectedItem.Text != "ALL")
        {
    ...
    protected void ddlArea_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        String strQuery = "SELECT CategoryID,DAID,Name,ZoneName FROM VW_Delear_Info ";
        strQuery = strQuery + " WHERE (Discontinue = 'No')";
        strQuery = strQuery + " AND (ZoneName='" + ddlArea.SelectedItem.Text + "')";
        strQuery = strQuery + " ORDER BY Name";
        //SqlConnection con = new SqlConnection("conn");
        SqlCommand cmd = new SqlCommand(strQuery, conn);
        //SqlCommand cmd = new SqlCommand();
