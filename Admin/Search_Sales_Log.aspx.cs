...
public partial class Search_Sales_Log : System.Web.UI.Page
{
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        ...
        sSql = "SELECT dbo.HistoryMaster.HisMID, dbo.HistoryMaster.MRSRMID, dbo.HistoryMaster.MRSRCode," +
            " CONVERT(varchar(12), TDate, 101) AS TDate, dbo.HistoryMaster.OutSource," +
            "NetSalesAmnt," +
            ...
            " dbo.HistoryMaster.Customer, dbo.Customer.CustID," +
            ...
            " FROM dbo.HistoryMaster LEFT OUTER JOIN" +
            " dbo.Customer ON dbo.HistoryMaster.Customer = dbo.Customer.Mobile" +
            " WHERE (dbo.HistoryMaster.MRSRMID = '" + sPID + "')" +
            " AND (dbo.HistoryMaster.TrType = 3)";
        //" AND (dbo.HistoryMaster.OutSource='" + Session["sBrId"] + "')";
        ...
        if (dr.Read())
        {
            ...
            //this.lblID.Text = dr["HisMID"].ToString();
        ...
        sSql = "SELECT dbo.Product.ProductID, dbo.Product.Model," +
            " dbo.Product.MRP," +
            " dbo.HistoryDetails.UnitPrice AS CampaignPrice," +
            " ABS(dbo.HistoryDetails.Qty) AS Qty," +
            " dbo.HistoryDetails.TotalAmnt As TotalPrice," +
            " dbo.HistoryDetails.DiscountAmnt AS DisAmnt, " +
            " dbo.HistoryDetails.DisCode, dbo.HistoryDetails.DisRef," +
            " ISNULL(dbo.HistoryDetails.WithAdjAmnt,0) AS WithAdjAmnt, dbo.HistoryDetails.NetAmnt," +
            " dbo.HistoryDetails.SLNO AS ProductSL," +
            " dbo.HistoryDetails.ProdRemarks as Remarks" +
            " FROM dbo.Product INNER JOIN" +
            " dbo.HistoryDetails ON dbo.Product.ProductID = dbo.HistoryDetails.ProductID" +
            " WHERE (dbo.HistoryDetails.MRSRMID = '" + this.lblID.Text + "')";
    ...
    protected void lnkDel_Click(object sender, EventArgs e)
    {
        ...
        */
        ...
        //sSql = "";
        //sSql = "DELETE FROM HistoryMaster";
        ...
        //sSql = "";
        //sSql = "DELETE FROM HistoryDetails";
        //sSql = sSql + " WHERE MRSRMID='" + sMasterID + "'";
    ...
    private void fnLoadData()
    {
        ...
        string sSql = "";
        sSql = "SELECT HisMID, MRSRMID, MRSRCode, CONVERT(varchar(12), TDate, 105) AS TDate, ";
        sSql = sSql + " ISNULL(NetSalesAmnt,0) AS NetSalesAmnt,  dbo.Entity.eName as Entity,";
        sSql = sSql + " ISNULL(CashAmnt,0) AS CashAmnt, dbo.HistoryMaster.EntryDate,";
        ...
        sSql = sSql + " FROM dbo.Entity INNER JOIN  dbo.HistoryMaster ON dbo.Entity.EID = dbo.HistoryMaster.OutSource LEFT OUTER JOIN ";
        sSql = sSql + " dbo.Customer ON dbo.HistoryMaster.Customer = dbo.Customer.Mobile";

        sSql = sSql + " WHERE (dbo.HistoryMaster.TrType = 3) ";

        if (ddlEntity.SelectedItem.Text != "ALL")
        {
            sSql = sSql + " AND (dbo.HistoryMaster.OutSource='" + ddlEntity.SelectedItem.Value  + "')";
        }

        sSql = sSql + " AND (dbo.HistoryMaster.TDate >= '" + Convert.ToDateTime(this.txtFrom.Text) + "'";
        sSql = sSql + " AND dbo.HistoryMaster.TDate <= '" + Convert.ToDateTime(this.txtToDate.Text) + "')";

        sSql = sSql + " ORDER BY TDate, MRSRCode, HisMID DESC";
