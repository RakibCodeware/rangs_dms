...
public partial class Deposite_Entry : System.Web.UI.Page
{
    ...
    protected void LoadProductGroupList()
    {
        ...
        String strQuery = "select GroupSL, GroupName from Product" +
            " where GroupSL not in (2,3,4,7,8,9,10,11,12,13,14,15,20,21,22,24,26,27,31,32,33,34,38,40,41,43,52,53,54,55,56,57,59," +
    ...
    protected void fnLoadData_DSM()
    {
        ...
        sSql = sSql + " dbo.DepositAmnt.cRemarks, dbo.Zone.CatName AS ZoneName, dbo.Zone.CategoryID, dbo.DelearInfo.DAID, dbo.DepositAmnt.ProductGroup";
    ...
    protected void btnSaveClick(object sender, EventArgs e)
    {
        ...
        sSql = "INSERT INTO tbDeposit(RefNo,DepositDate,EID,CollectionNo," +
               "BankID,BankName,BranchName,DepositAmnt,DepositType,DepositBy,Remarks,POSMachine,UserID,EntryDate,ProductGroup)" +
                     ...
                     " '" + Session["UserID"] + "', '" + DateTime.Today + "', '" + ddlProductGroup.SelectedItem.Text + "'" +
                     " )";
        SqlCommand cmd = new SqlCommand(sSql, conn);
        conn.Open();
        cmd.ExecuteNonQuery();
        ...
        sSql = sSql + " UserID,PCName,EntryDate,ProductGroup) VALUES(";
        ...
        sSql = sSql + " '" + Session["UserID"] + "','Online','" + DateTime.Today + "', '" + ddlProductGroup.SelectedItem.Text + "')";
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        sSql = sSql + " UserID='" + Session["UserID"] + "',EntryDate='" + DateTime.Today + "'";
    ...
    public static object getDataFromRangsConnect(FormData formdata)
    {
        try
        {
         using (SqlConnection conn = DBConnectionDSM.GetConnection())
                {
                    string sSql = "INSERT INTO tbDeposit(RefNo,DepositDate,CollectionNo)" +
                     " Values ('" +formdata.RefNo + "','" + formdata.DepositDate + "','" + formdata.CollectionNo + "')";
          ...
          return new
          {
             Message = "Received: " + formdata.BankName + ", " + formdata.DepositType + ", " + formdata.DepositAmount
          };
        }
        catch(Exception ex)
        {
    ...
    public static string showData()
    {
        ...
        try
        {
            using (SqlConnection conn = DBConnectionDSM.GetConnection())
            {
                string sSql = "select Name from VW_Delear_Info where ZoneName='DHAKA NORTH'";
                using (SqlCommand cmd = new SqlCommand(sSql, conn))
                {
             ...
             // Return the list of dealer names
        }
        catch (Exception ex)
        {
            return null;
