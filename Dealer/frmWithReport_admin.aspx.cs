...
public partial class FormsReport_Admin_frmWithReport_Admin : System.Web.UI.Page
{
    ...
    private void fnLoadReceiveSummary_GroupWise()
    {
        ...
        {
            sSql = sSql + " AND (dbo.Product.GroupName = '" + ddlGroup.SelectedItem.ToString() + "') ";
        }
    ...
    protected void GridView3_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            CalcTotalQty1(e.Row.Cells[4].Text);
    ...
    private void fnLoadReceiveSummary_ChallanWise()
    {
        ...
        string sSql = "";
        sSql = "SELECT MRSRCode, CONVERT(varchar(12), TDate, 105) AS TDate, NetSalesAmnt," +
            " Remarks" +
            " FROM dbo.MRSRMaster" +
            " WHERE (dbo.MRSRMaster.TrType = -3) " +
            " AND (dbo.MRSRMaster.InSource='" + Session["sBrId"] + "')";
