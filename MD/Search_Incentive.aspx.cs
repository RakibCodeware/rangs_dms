...
public partial class Search_Incentive : System.Web.UI.Page
{
    ...
    //LOAD MODEL IN DROPDOWN LIST
    protected void LoadDropDownList_Model()
    {
        SqlConnection conn = DBConnection.GetConnection();

        String strQuery = "select ProductID, Model from Product ";
        strQuery = strQuery + " WHERE (Discontinue = 'No') AND (GetIncentive = 1)";
    ...
    private void fnLoadData_Summary_ItemWise()
    {
        ...
        sSql = sSql + " SUM(ABS(dbo.MRSRDetails.Qty)) AS tQty, ";
        ...
        {
            sSql = sSql + " AND (dbo.Product.Model='" + ddlModel.SelectedItem.Text + "')";
        }

        sSql = sSql + " GROUP BY dbo.MRSRMaster.TrType, dbo.Product.Model, dbo.Product.ProdName,";
        sSql = sSql + " dbo.Product.GetIncentive";
    ...
    private void fnLoadData_Summary()
    {
        ...
        sSql = "SELECT dbo.MRSRMaster.TrType, dbo.Product.GetIncentive, dbo.Entity.eName,";
        //sSql = sSql + " SUM(dbo.MRSRDetails.IncentiveAmnt) AS tIncentive, dbo.Entity.eName";
        sSql = sSql + " SUM(dbo.MRSRDetails.NetAmnt - dbo.MRSRDetails.BLIPAmnt * ABS(dbo.MRSRDetails.Qty)) AS tIncentive";
        sSql = sSql + " FROM  dbo.Product INNER JOIN";
        sSql = sSql + " dbo.MRSRDetails ON dbo.Product.ProductID = dbo.MRSRDetails.ProductID INNER JOIN";
        sSql = sSql + " dbo.MRSRMaster ON dbo.MRSRDetails.MRSRMID = dbo.MRSRMaster.MRSRMID INNER JOIN";
        ...
        sSql = sSql + " WHERE (dbo.Product.GetIncentive = 1)";
        sSql = sSql + " AND (dbo.MRSRMaster.TrType = 3)";
        //sSql = sSql + "  AND (dbo.MRSRMaster.TDate >= CONVERT(DATETIME, '2020-02-20 00:00:00', 102))";
        ...
        sSql = sSql + " GROUP BY dbo.MRSRMaster.TrType, dbo.Product.GetIncentive, dbo.Entity.eName";
    ...
    protected void GridView2_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            CalcTotal_Incentive(e.Row.Cells[2].Text);
            //CalcTotal_Cash(e.Row.Cells[6].Text);
        ...
        {
            e.Row.Cells[1].Text = "Total";            
            e.Row.Cells[2].Text = runningTotalIncentive.ToString("0,0", CultureInfo.InvariantCulture);
            //e.Row.Cells[6].Text = runningTotalCash.ToString("0,0", CultureInfo.InvariantCulture);
