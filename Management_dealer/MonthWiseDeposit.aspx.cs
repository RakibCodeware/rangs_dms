...
public partial class MonthWiseDeposit : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            lblMSales.Text = sFYs + " to " + sFYe;
            //lblMSales.Text = Session["sFY"].ToString();
    ...
    private void fnPick_Load()
    {
        ...
        for (int i= 1; i <= 12; i++)
        {
            ...
            gSql = "SELECT SUM(ISNULL(dbo.tbDeposit.DepositAmnt,0)) AS TAmnt";
            //gSql = gSql + " FROM dbo.tbDeposit";
            gSql = gSql + " FROM dbo.tbDeposit INNER JOIN dbo.Entity ON dbo.tbDeposit.EID = dbo.Entity.EID";
            gSql = gSql + " WHERE ((Month(dbo.tbDeposit.DepositDate) = '" + i + "') AND (Year(dbo.tbDeposit.DepositDate) = '" + FYs + "'))";
            gSql = gSql + " AND (dbo.Entity.EntityType = 'Dealer')";
            //gSql = gSql + " GROUP BY DepositDate";
    ...
    private void fnLoadData_in_grid()
    {
        ...
        */
        ...
        sSql = "";
        sSql = "SELECT MonthName, ISNULL(SalesAmnt,0) AS DepositAmnt,";
